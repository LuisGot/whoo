...
import { formatOverview } from "../src/format";
import type { OverviewPayload } from "../src/types";

describe("formatOverview", () => {
  test("renders readable labels and value formats", () => {
    ...
    expect(output).toContain("Average Heart Rate: 72 bpm");
    expect(output).toContain("Maximum Heart Rate: 139 bpm");
    expect(output).toContain("Recovery:");
    expect(output).toContain("Recovery Score: 78%");
    expect(output).toContain("Resting Heart Rate: 56 bpm");
    expect(output).toContain("HRV (RMSSD): 28.94 ms");
    expect(output).toContain("Blood Oxygen (SpO2): 98.05%");
    expect(output).toContain("Skin Temperature: 33.87 °C");
    expect(output).toContain("Sleep:");
    expect(output).toContain("Nap: No");
    expect(output).toContain("Time In Bed: 7h 30m");
    expect(output).toContain("Awake Time: 6m 30s");
    expect(output).toContain("No Data Time: 0s");
    expect(output).toContain("Need Reduction From Recent Nap: -2m");
    expect(output).toContain("Respiratory Rate: 17.71 breaths/min");
    expect(output).toContain("Sleep Performance: 92%");
    expect(output).toContain("Sleep Consistency: 60%");
  ...
  test("shows no data for metadata-only records", () => {
    const payload: OverviewPayload = {
